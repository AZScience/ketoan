...
interface GoodsReceivedPreviewProps {
  voucher: any;
  companySettings: any;
}

export const GoodsReceivedPreview: React.FC<GoodsReceivedPreviewProps> = ({ voucher, companySettings }) => {
  const { number, date, metadata, items = [] } = voucher;
  const subTotal = items.reduce((sum: number, item: any) => sum + Number(item.amount || 0), 0);
  const totalVat = items.reduce((sum: number, item: any) => sum + (Number(item.vatAmount) || 0), 0);
  ...
  return (
    <div className="p-12 bg-white text-black font-serif text-[10px] leading-relaxed border border-slate-200 shadow-inner max-w-[800px] mx-auto print:p-0 print:border-0 print:shadow-none print:max-w-none">
      <VoucherHeader 
        ...
        templateCode="Mẫu số 01 - VT"
        templateSource="Thông tư số 99/2018/TT-BTC"
        templateDate="Ngày 01/11/2018 của Bộ Tài chính"
      />

      <div className="text-center mb-6">
        <h2 className="text-xl font-bold uppercase tracking-wider">PHIẾU NHẬP KHO</h2>
      ...
      <div className="space-y-2 mb-6">
        <p>- Họ và tên người giao: <span className="font-bold">{metadata?.deliverer || '................................................................................................'}</span></p>
        <p>- Theo: <span className="font-bold">{metadata?.referenceDoc || '................................................................................................'}</span></p>
        <p>- Nhập tại kho: <span className="font-bold">{metadata?.warehouse || '................'}</span> địa điểm <span className="font-bold">{metadata?.location || '................'}</span></p>
      </div>
      ...
      <table className="w-full border-collapse border border-black mb-6 text-[9px]">
        <thead>
          ...
          <tr className="font-bold text-center bg-slate-50 print:bg-transparent">
            ...
            <th className="border border-black p-1 w-14">Thực nhập</th>
          </tr>
        </thead>
        <tbody>
          {items.map((item: any, i: number) => (
            <tr key={i} className="h-8">
              ...
              <td className="border border-black p-1">{item.description || item.name}</td>
              <td className="border border-black p-1 text-center">{item.itemCode || item.code}</td>
          ...
          <tr className="font-bold h-8">
            <td className="border border-black p-1 text-right" colSpan={7}>Cộng tiền hàng</td>
            <td className="border border-black p-1 text-right">{subTotal.toLocaleString()}</td>
          </tr>
          <tr className="font-bold h-8">
            <td className="border border-black p-1 text-right" colSpan={7}>Tiền thuế GTGT</td>
            <td className="border border-black p-1 text-right">{totalVat.toLocaleString()}</td>
          </tr>
          <tr className="font-bold h-10 bg-slate-50 print:bg-transparent">
            <td className="border border-black p-1 text-right uppercase" colSpan={7}>Tổng cộng tiền thanh toán</td>
            <td className="border border-black p-1 text-right text-lg">{totalAmount.toLocaleString()}</td>
          </tr>
        </tbody>
      </table>
      ...
      <div className="space-y-2 mb-8">
        ...
        <p>- Số chứng từ gốc: <span className="font-bold">{metadata?.attachments || '............'}</span></p>
      </div>

      <VoucherSignatures
        roles={[
          { title: 'Người lập phiếu', subtitle: '(Ký, họ tên)' },
          { title: 'Người giao hàng', subtitle: '(Ký, họ tên)' },
          { title: 'Thủ kho', subtitle: '(Ký, họ tên)' },
          { title: 'Kế toán trưởng', subtitle: '(Hoặc bộ phận có nhu cầu nhập)', isDate: true },
        ]}
        date={date}
      />

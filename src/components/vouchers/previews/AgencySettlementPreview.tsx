import React from 'react';
import { format } from 'date-fns';
import { VoucherHeader } from './VoucherHeader';
import { VoucherSignatures } from './VoucherSignatures';

interface AgencySettlementPreviewProps {
  voucher: any;
  companySettings: any;
}

export const AgencySettlementPreview: React.FC<AgencySettlementPreviewProps> = ({ voucher, companySettings }) => {
  const { number, date, metadata } = voucher;
  const rows = metadata?.rows || [];
  const totalAmount = (rows || []).reduce((sum: number, row: any) => sum + (Number(row.amount) || 0), 0);

  return (
    <div className="p-12 bg-white text-black font-serif text-xs leading-relaxed border border-slate-200 shadow-inner max-w-[800px] mx-auto print:p-0 print:border-0 print:shadow-none print:max-w-none">
      <VoucherHeader
        companyName={companySettings?.name}
        companyAddress={companySettings?.address}
        templateCode="Mẫu số 01 - BH"
        templateSource="Thông tư số 200/2014/TT-BTC"
        templateDate="Ngày 22/12/2014 của Bộ Tài chính"
      />

      <div className="text-center mb-10">
        <h2 className="text-2xl font-bold uppercase tracking-wider">BẢNG THANH TOÁN HÀNG ĐẠI LÝ, KÝ GỬI</h2>
        <p className="italic text-[11px]">Tháng {date ? format(new Date(date), 'MM') : '...'} năm {date ? format(new Date(date), 'yyyy') : '...'}</p>
        <p className="mt-1 font-bold">Số: {number || '............'}</p>
      </div>

      <div className="mb-6 space-y-1">
        <p><span className="font-bold">Tên đại lý:</span> {metadata?.agencyName || '................................................................'}</p>
        <p><span className="font-bold">Địa chỉ:</span> {metadata?.agencyAddress || '................................................................'}</p>
      </div>

      <table className="w-full border-collapse border border-black mb-6 text-[10px]">
        <thead>
          <tr className="font-bold text-center bg-slate-50 print:bg-transparent">
            <th className="border border-black p-2 w-12">STT</th>
            <th className="border border-black p-2">Tên hàng hóa</th>
            <th className="border border-black p-2 w-16">ĐVT</th>
            <th className="border border-black p-2 w-16">SL giao</th>
            <th className="border border-black p-2 w-16">SL bán</th>
            <th className="border border-black p-2 w-24">Đơn giá</th>
            <th className="border border-black p-2 w-28">Thành tiền</th>
            <th className="border border-black p-2 w-24">Hoa hồng</th>
          </tr>
        </thead>
        <tbody>
          {(rows || []).map((row: any, i: number) => (
            <tr key={i} className="h-8">
              <td className="border border-black p-2 text-center">{i + 1}</td>
              <td className="border border-black p-2">{row.name}</td>
              <td className="border border-black p-2 text-center">{row.unit}</td>
              <td className="border border-black p-2 text-center">{row.deliveredQty}</td>
              <td className="border border-black p-2 text-center">{row.soldQty}</td>
              <td className="border border-black p-2 text-right">{Number(row.price || 0).toLocaleString()}</td>
              <td className="border border-black p-2 text-right font-bold">{Number(row.amount || 0).toLocaleString()}</td>
              <td className="border border-black p-2 text-right">{Number(row.commission || 0).toLocaleString()}</td>
            </tr>
          ))}
          <tr className="font-bold h-10">
            <td className="border border-black p-2 text-center" colSpan={6}>Tổng cộng</td>
            <td className="border border-black p-2 text-right">{totalAmount.toLocaleString()}</td>
            <td className="border border-black p-2"></td>
          </tr>
        </tbody>
      </table>

      <VoucherSignatures
        roles={[
          { title: 'Người lập biểu', subtitle: '(Ký, họ tên)' },
          { title: 'Kế toán trưởng', subtitle: '(Ký, họ tên)' },
          { title: 'Đại diện đại lý', subtitle: '(Ký, họ tên)', isDate: true }
        ]}
        gridCols={3}
        date={date}
      />
    </div>
  );
};

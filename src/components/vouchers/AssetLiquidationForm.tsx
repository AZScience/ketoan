...
import { Plus, Trash2, Hash, Box, User, Briefcase, FileText, ClipboardList, AlertTriangle } from 'lucide-react';

interface AssetLiquidationRow {
  stt: string;
  assetName: string;
  assetCode: string;
  unit: string;
...
interface AssetLiquidationFormProps {
  metadata: any;
  onChange: (metadata: any) => void;
  isViewOnly?: boolean;
}

const AssetLiquidationForm: React.FC<AssetLiquidationFormProps> = ({ metadata, onChange, isViewOnly }) => {
  const [rows, setRows] = useState<AssetLiquidationRow[]>(metadata?.rows || [
    {
      stt: '1',
      assetName: '',
      assetCode: '',
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <textarea
              ...
              onChange={(e) => onChange({ ...metadata, committee: e.target.value })}
              disabled={isViewOnly}
              ...
              placeholder="Nhập danh sách ban thanh lý..."
            />
          </div>
          <div className="space-y-2">
            <label className="text-sm font-bold text-slate-700 flex items-center gap-2">
...
export default AssetLiquidationForm;
